...
do
	...
	echo "Run zap2xml.pl"
	/zap2xml.pl -u $USERNAME -p $PASSWORD -U -o /tmp/xmltv/raws/1.xml -c cache1 -k "${URL_ASSETS%/}/" $OPT_ARGS
	...
	then
		...
		/zap2xml.pl -u ${USERNAME2} -p ${PASSWORD2} -U -o /tmp/xmltv/raws/2.xml -c cache2 -k "${URL_ASSETS%/}/" $OPT_ARGS2
